...
from .search_cells     import SearchCell_PartialChannel
...
class TinyNetworkPCDarts(nn.Module):
  ...
  def __init__(self, C, N, max_nodes, num_classes, search_space):
    ...
    for index, (C_curr, reduction) in enumerate(zip(layer_channels, layer_reductions)):
      if reduction:
        ...
        cell = SearchCell_PartialChannel(C_prev, C_curr, 1, max_nodes, search_space)
  ...
  def forward(self, inputs, pert=None):
    ...
    idx = 0
    for i, cell in enumerate(self.cells):
      if isinstance(cell, SearchCell_PartialChannel):
        w = alphas.clone()
        if self.training:
          w.retain_grad()
  ...
  def get_perturbations(self):
    ...
    with torch.no_grad():
      ...
      P = [(1 / g.norm(p=2.0)) * (I - torch.outer(u_l, u_l)) for g, u_l in zip(layer_gradients, u)]
